...
router.get("/", verifyToken, async (req, res) => {
  try {
    ...
    // 🐛 ที่แก้: เดิมบรรทัดนี้อ้าง `error` ซึ่งไม่มีอยู่จริง (ตัวแปรที่ catch มาชื่อ `err`) — พอเข้า catch
    // มันจะโยน ReferenceError ซ้อนขึ้นมาทันที ทำให้บรรทัด res.status(500) ข้างล่าง "ไม่ถูกเรียกเลย"
    // ผลคือ request ค้างจนหมดเวลาแทนที่จะได้ 500 กลับไป (ฝั่งเบราว์เซอร์เห็นเป็นหมุนค้าง ไม่มี error)
...
router.delete("/:id", verifyToken, async (req, res) => {
  try {
    ...
    if (productToDelete.userId !== req.userId && !canEditMasterData(req.user)) {
