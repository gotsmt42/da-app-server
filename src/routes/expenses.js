 ...
 *
 * สายงาน 3 ส่วน (ผู้ใช้กำหนด — ใช้เหมือนกันทุกชนิดใบ, ใครทำอะไรได้ดูที่ config/roles.js):
 *   ส่วนที่ 1 ส่งขอเบิก                       → แจ้งแอดมินช่าง + ผู้จัดการแผนกช่าง
 *   ส่วนที่ 2 ตรวจสอบ/อนุมัติ                  → ⚠️ ยังเป็นสองมือเหมือนเดิม: แอดมินช่าง "ตรวจสอบ" แล้วส่งต่อให้
 *             (ตรวจสอบ → อนุมัติ)                ผู้จัดการแผนกช่าง "อนุมัติ" (คนละสถานะ คนละลายเซ็นในระบบ)
 *                                                แต่บนเอกสาร/หน้าจอ นับรวมเป็นส่วนเดียว ตามที่ผู้ใช้สั่ง
 ...
 */
/**
 * ⚠️ นับเป็น 3 ส่วน — "ตรวจสอบ" กับ "อนุมัติ" เป็นส่วนที่ 2 ด้วยกันทั้งคู่ (คนละมือ คนละสถานะเหมือนเดิม)
 * ผู้ใช้สั่งให้เอกสารและการนับขั้นเหลือ 3 ส่วน แต่ "แอดมินตรวจสอบ แล้วส่งให้ผู้จัดการอนุมัติ" ยังเหมือนเดิม
 */
const TOTAL_STEPS = 3;
const STEP = {
  review: { no: 2, cap: "reviewExpense", label: "ตรวจสอบ" },
  approve: { no: 2, cap: "approveExpense", label: "อนุมัติ" },
  disburse: { no: 3, cap: "disburseExpense", label: "อนุมัติเบิกจ่าย" },
};
