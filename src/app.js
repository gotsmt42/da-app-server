...
// ✅ สิทธิ์ที่ผู้ดูแลปรับเองจากหน้าตั้งค่า — โหลดเข้าหน่วยความจำให้ can() ใช้ แล้วรีเฟรชเป็นระยะ
// ⚠️ ต้องเริ่มหลังประกอบ route เสร็จ (ไม่มีผลกับลำดับ middleware) และไม่ await — ระหว่างรอโหลด
// ระบบใช้ตารางค่าเริ่มต้นในโค้ดไปก่อน ซึ่งเป็นค่าที่ปลอดภัยกว่าเสมอ
require("./services/permissionOverrides").startPermissionOverrideRefresh();

// ✅ ย้ายชื่อฟิลด์ผู้ใช้ให้ตรงกับคำที่ใช้ทั้งระบบ (rank = ตำแหน่งในองค์กร · role = ตำแหน่งในระบบ)
// ⚠️ รันซ้ำได้ ไม่ต้อง await — โค้ดอ่านได้ทั้งรูปแบบเก่าและใหม่ ระหว่างย้ายจึงไม่มีช่วงที่ระบบใช้งานไม่ได้
