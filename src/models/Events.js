...
const eventSchema = new mongoose.Schema(
  {
    ...
    /**
     * ป้ายกำกับแผนกเจ้าของสัญญา — "ข้อมูลประกอบ" ล้วนๆ สำหรับดู/กรอง/ออกรายงานในหน้าภาพรวมงาน
     *
     * ⚠️ คนละตัวกับ department ด้านบนโดยตั้งใจ ห้ามยุบรวมกัน: department คุม "ใครเห็นงานนี้บ้าง"
     * (departmentScope กรองทุก query) ส่วนตัวนี้ไม่ถูกใช้ในเงื่อนไขการกรองใดๆ ทั้งสิ้น — ติดป้ายว่า
     * สัญญานี้เป็นของฝ่ายขายได้ โดยงานยังอยู่ในปฏิทิน/หน้าดำเนินงานของช่างครบเหมือนเดิมทุกใบ
     *
     * ⚠️ ไม่ตั้ง default โดยตั้งใจ — งานเก่าที่ยังไม่เคยติดป้ายจะอ่านเป็น "ฝ่ายบริการ" ที่ฝั่งแสดงผล
     * (ดู groupEventsByContract) จึงไม่ต้อง migrate ข้อมูลเดิมเลยสักใบ
     */
    departmentTag: { type: String, enum: ["service", "sales"] },

    /**
     * หมายเหตุสถานะสัญญาที่คนกรอกพิมพ์เอง — ทับข้อความสถานะที่ระบบคำนวณให้ในคอลัมน์ "สถานะสัญญา"
     *
     * ⚠️ ไม่มี enum โดยตั้งใจ: ข้อความอิสระ เช่น "รอลูกค้าเซ็นกลับ" / "ต่อสัญญาแล้ว รอเอกสาร" ซึ่งเป็น
     * สถานะจริงในการทำงานที่ระบบไม่มีทางเดาเองได้จากวันที่ในสัญญา
     * ⚠️ ไม่ถูกใช้ในเงื่อนไขการกรอง/แจ้งเตือนใดๆ ฝั่ง server — สถานะหมดอายุ/ใกล้หมดอายุยังคำนวณจาก
     * contractEnd เหมือนเดิมทุกจุด (ดู services/OverdueReminder.js) ตัวนี้เป็นข้อความให้คนอ่านล้วนๆ
     */
    statusNote: { type: String },

    /**
     * หมายเหตุทั่วไปของสัญญา/งาน — ข้อความอิสระที่คนทำงานจดไว้เอง เช่น "ลูกค้าขอเลื่อนรอบ 2 ไปหลังสงกรานต์"
     * / "ต้องแจ้ง รปภ. ล่วงหน้า 1 วัน" ซึ่งเป็นข้อมูลที่ไม่มีช่องไหนในระบบรองรับแต่จำเป็นต้องส่งต่อกัน
     *
     * ⚠️ คนละตัวกับ statusNote ด้านบนโดยตั้งใจ: statusNote คือ "สถานะของสัญญาตอนนี้คืออะไร" ซึ่งไปแทนที่
     * ข้อความสถานะอัตโนมัติในคอลัมน์สถานะ ส่วนตัวนี้เป็นบันทึกทั่วไปที่ไม่เกี่ยวกับสถานะ — ถ้ายุบรวมกัน
     * การจดโน้ตธรรมดาจะไปกลบสถานะหมดอายุ/ใกล้หมดอายุบนหน้าจอทันที
     * ⚠️ ไม่ถูกใช้ในเงื่อนไขการกรอง/แจ้งเตือนใดๆ ฝั่ง server — เป็นข้อความให้คนอ่านล้วนๆ
     */
    remark: { type: String },

    /**
     * ผู้ติดต่อหน้างาน — ชื่อ + เบอร์โทรของคนที่ช่างต้องโทรหาเมื่อไปถึงหน้างาน
     *
     * ⚠️ เก็บไว้ที่ "งาน" ไม่ใช่อ่านสดจากทะเบียนลูกค้าอย่างเดียว เพราะผู้ติดต่อของ "งานครั้งนี้" อาจไม่ใช่
     * ผู้ติดต่อหลักของโครงการ (เช่นงานนี้ให้ประสานกับ ผจก.อาคาร คนละคนกับที่ลงทะเบียนไว้) และคนใน
     * ทะเบียนอาจเปลี่ยนทีหลัง ซึ่งต้องไม่ย้อนไปแก้ประวัติงานเก่าที่เคยติดต่อคนเดิมไว้
     * ✅ ตอนสร้างงานใหม่ ฝั่งหน้าจอเติมค่าจากทะเบียนลูกค้า (Customer.cName / Customer.tel) ให้อัตโนมัติ
     * เป็นค่าตั้งต้น แล้วแก้ทับเฉพาะงานนั้นได้ — ดู applyCustomerContact ใน EventForms
     * ✅ ใบแจ้งงานจากฝ่ายขาย (Dispatch) มีข้อมูลนี้อยู่แล้ว ตอนอนุมัติจะถูกคัดลอกมาที่นี่ด้วย
     * (ดู routes/dispatch.js) ไม่ต้องให้ช่างมากรอกซ้ำเอง
     */
    contactName: { type: String, default: "" },
    contactTel: { type: String, default: "" },

    /**
     * ลำดับการแสดงในช่องวันเดียวกันบนปฏิทิน — ผู้ใช้ลากสลับเองได้ว่าอยากให้งานไหนอยู่บน/ล่าง
     *
     * ✅ เก็บที่ตัวงาน (ไม่ใช่ค่าความชอบรายคน) โดยตั้งใจ — ลำดับงานในวันหนึ่งคือ "จะทำอันไหนก่อน"
     * ซึ่งเป็นข้อมูลของทีม ทุกคนที่เปิดดูวันนั้นต้องเห็นลำดับเดียวกัน ไม่ใช่ต่างคนต่างเรียง
     * ⚠️ ค่าเริ่มต้น 0 ทุกใบ — งานเก่าทั้งระบบจึงเรียงเหมือนเดิมทุกประการ (ตกไปใช้เกณฑ์สำรองคือเวลา
     * เริ่ม/ชื่อเรื่อง เหมือนก่อนมีฟิลด์นี้) จนกว่าจะมีคนลากจัดลำดับวันนั้นเป็นครั้งแรก
     */
    ...
    billing: {
      ...
      // ⚠️ ห้ามใส่ index: true ตรงนี้ — index ของฟิลด์นี้ประกาศไว้ท้ายไฟล์แบบ sparse แล้ว
      // (eventSchema.index({ "billing.dueAt": 1 }, { sparse: true })) ถ้าใส่ทั้งสองที่ Mongoose จะ
      // สร้าง index ซ้อนกัน 2 ชุด + เตือน "Duplicate schema index" ทุกครั้งที่เซิร์ฟเวอร์เริ่ม
      // และตัวที่ไม่ sparse จะกิน index ไปกับค่า null ของงานที่ยังไม่ได้วางบิล (ส่วนใหญ่) ฟรีๆ
