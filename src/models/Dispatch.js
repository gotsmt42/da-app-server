...
const dispatchSchema = new mongoose.Schema(
  {
    ...
    /**
     * ── งานตามสัญญา (ผูกได้ ไม่บังคับ) ──────────────────────────────
     * ✅ เซลแจ้ง "ครั้งถัดไปของสัญญาที่มีอยู่แล้ว" ได้ ไม่ใช่แค่งานครั้งเดียวจบ — เดิมใบแจ้งงานทุกใบ
     * ถูกสร้างเป็นงานทั่วไป (jobClassification: "general") เสมอ ทำให้งานที่จริงๆ เป็นครั้งที่ N ของ
     * สัญญา หลุดจากสัญญาไปเป็นงานลอย นับครั้งไม่ตรง และไม่โผล่ในหน้า "ภาพรวมงาน"
     *
     * ⚠️ เก็บเป็น "สำเนา ณ เวลาที่แจ้ง" (snapshot) ไม่ใช่ join ตอนแสดงผล — ใบยังอ่านรู้เรื่องแม้สัญญา
     * ต้นทางถูกแก้/ลบภายหลัง เทียบแบบแผนเดียวกับ customer ด้านบนที่ก๊อปชื่อบริษัทมาเก็บไว้เอง
     * ⚠️ ไม่เก็บ "ครั้งที่" (time) ตรงนี้ — ครั้งที่ถูกจองตอน *อนุมัติ* เท่านั้น (ดู POST /:id/approve)
     * ถ้าให้เซลจองไว้ตั้งแต่ตอนแจ้ง ครั้งนั้นอาจถูกงานอื่นใช้ไปก่อนที่แอดมินจะกดอนุมัติ
     */
    contract: {
