...
## สิทธิ์: Role กับ Rank (และกำหนดเองใน MongoDB ยังไง)

ระบบมีสิทธิ์ **สองชั้นที่แยกขาดจากกัน** — เอกสารผู้ใช้หนึ่งคนใน collection `users` มี 3 ช่องที่เกี่ยวข้อง:

| ช่อง | คืออะไร | ค่าที่ใส่ได้ |
|------|---------|----------------|
| `rank` | **Rank — ตำแหน่งในองค์กร** กำหนดว่าทำงานอะไรได้บ้าง (ตารางสิทธิ์) | `director` · `manager` · `admin` · `techlead` · `technician` · `sale` · `user` |
| `role` | **Role — ตำแหน่งในระบบ** กำหนดว่าดูแลระบบได้แค่ไหน | `superadmin` · `admin` · `member` |
| `jobTitle` | ตำแหน่งเฉพาะบุคคลที่พิมพ์ใต้ชื่อในเอกสาร (ไม่ให้สิทธิ์อะไรเลย) | ข้อความอิสระ เช่น `"ช่างเทคนิคอาวุโส"` (เว้นว่าง = ใช้ชื่อ Rank) |

**กำหนดสิทธิ์จาก MongoDB ตรงๆ** (เช่นกู้คืนตอนไม่มีใครเข้าหน้าตั้งค่าได้):

```js
// ให้คนนี้เป็นผู้ดูแลระบบสูงสุด (Role) — ตำแหน่งในองค์กรคงเดิม
db.users.updateOne({ username: "admin" }, { $set: { role: "superadmin" } })

// ย้ายคนนี้ไปเป็นแอดมินช่าง (Rank) — สิทธิ์ดูแลระบบคงเดิม
db.users.updateOne({ username: "somchai" }, { $set: { rank: "admin" } })

// ตั้งตำแหน่งที่พิมพ์ใต้ชื่อในเอกสาร
db.users.updateOne({ username: "somchai" }, { $set: { jobTitle: "หัวหน้าธุรการ" } })
```

⚠️ หลังแก้ในฐานข้อมูลตรงๆ คนนั้นต้อง **ออกแล้วเข้าใหม่** ถึงจะได้สิทธิ์ใหม่ (หน้าจอจำค่าไว้ในเครื่อง)
⚠️ วิธีปกติควรตั้งจากหน้า **ตั้งค่า > ตั้งค่าสิทธิ์** แทน — มีการยืนยันรหัสผ่านและกันไม่ให้เหลือ Super Admin 0 คน

ข้อมูลเก่า (`role` = ตำแหน่งในองค์กร, `systemRole` = ตำแหน่งในระบบ) ยังอ่านได้เสมอ
และ `src/services/migrateUserFields.js` จะย้ายให้เองตอนเซิร์ฟเวอร์เริ่มทำงาน (รันซ้ำกี่รอบก็ได้)
